    ...
    #{
      ...
      values => [
        #{
          ...
          convert => fun(X) -> lists:nth(0 + 1, X) / 50 end
        },
        #{
          name => <<"Volumetric water content">>,
          convert => fun(X) -> 0.0000043 * math:pow(lists:nth(0 + 1, X)/50, 3) - 0.00055 * math:pow(lists:nth(0 + 1, X)/50, 2) + 0.0292 * (lists:nth(0 + 1, X)/50) - 0.053 end,
          unit => <<"m³⋅m⁻³"/utf8>>
        },
        #{
          name => <<"Soil temperature">>,
          convert => fun(X) -> (lists:nth(1 + 1, X) - 400) / 10 end,
          unit => <<"°C"/utf8>>
        },
        #{
...
start() ->
  io:format("~p~n", [decode("02031c00030037027100000c60")]),
  io:format("~p~n", [decode("02031c00020c60")]).
