...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(3, new List<SensorValue>() {
      new SensorValue("Dielectric permittivity", null, x => x[0] / 50),
      new SensorValue("Volumetric water content", "m³⋅m⁻³", x => 0.0000043 * Math.Pow(x[0]/50, 3) - 0.00055 * Math.Pow(x[0]/50, 2) + 0.0292 * (x[0]/50) - 0.053),
      new SensorValue("Soil temperature", "°C", x => (x[1] - 400) / 10),
      new SensorValue("Electrical conductivity", "µS⋅cm⁻¹", x => x[2] * 10)
    }),
