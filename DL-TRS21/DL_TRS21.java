...
class DL_TRS21_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Water potential", "kPa", new Conversion() {
...
public class DL_TRS21 {
    public static void main(String[] args) {
        String[] payloads = new String[] {
            "0201920003007580a10c25",
            "02019200020c25"
        };

        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_TRS21_Definition.SENSORS, pl);
