...
class DL_MES5_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(5, new SensorValue[] {
            new SensorValue("Status", null, new Conversion() {
            ...
            new SensorValue("Sludge blanket", "%", new Conversion() {
                public double execute(double x[]) { return x[2] / 100; }
            }),
            new SensorValue("Suspended solid", "g⋅L⁻¹", new Conversion() {
                public double execute(double x[]) { return x[3] / 100; }
            }),
            new SensorValue("Turbidity", "FAU", new Conversion() {
                public double execute(double x[]) { return x[4] / 10; }
            })
        }),
...
public class DL_MES5 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_MES5_Definition.SENSORS, pl);
