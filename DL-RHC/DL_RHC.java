...
class DL_RHC_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(4, new SensorValue[] {
            new SensorValue("Sensor ID", null, new Conversion() {
                public double execute(double x[]) { return x[0] + x[1] * 65536; }
            }),
...
public class DL_RHC {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_RHC_Definition.SENSORS, pl);
