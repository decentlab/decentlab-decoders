  ...
  [
    #{
      ...
      values => [
        #{
          name => <<"Pressure">>,
          convert => fun(X) -> (lists:nth(0 + 1, X) - 16384) / 32768 * (?Pmax - ?Pmin) + ?Pmin end,
          unit => <<"bar"/utf8>>
        },
        #{
          name => <<"Temperature">>,
          convert => fun(X) -> (lists:nth(1 + 1, X) - 384) * 0.003125 - 50 end,
          unit => <<"°C"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("02016700034e8060170c7f")]),
  io:format("~p~n", [decode("02016700020c7f")]).
