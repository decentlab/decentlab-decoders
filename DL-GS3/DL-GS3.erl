    ...
    #{
      ...
      values => [
        #{
          ...
          convert => fun(X) -> lists:nth(0 + 1, X) / 100 end
        },
        #{
          name => <<"Volumetric water content">>,
          convert => fun(X) -> 0.00000589 * math:pow(lists:nth(0 + 1, X)/100, 3) - 0.000762 * math:pow(lists:nth(0 + 1, X)/100, 2) + 0.0367 * (lists:nth(0 + 1, X)/100) - 0.0753 end,
          unit => <<"m³⋅m⁻³"/utf8>>
        },
        #{
...
start() ->
  io:format("~p~n", [decode("0203630003009980e100010c60")]),
  io:format("~p~n", [decode("02036300020c60")]).
