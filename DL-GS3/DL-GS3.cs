...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(3, new List<SensorValue>() {
      new SensorValue("Dielectric permittivity", null, x => x[0] / 100),
      new SensorValue("Volumetric water content", "m³⋅m⁻³", x => 0.00000589 * Math.Pow(x[0]/100, 3) - 0.000762 * Math.Pow(x[0]/100, 2) + 0.0367 * (x[0]/100) - 0.0753),
