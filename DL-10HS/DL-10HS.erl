    ...
    #{
      ...
      values => [
        #{
          ...
          convert => fun(X) -> 3 * (lists:nth(0 + 1, X) - 32768) / 32768 * 1000 end,
          unit => <<"mV"/utf8>>
        },
        #{
          name => <<"Volumetric water content">>,
          convert => fun(X) -> 2.97*math:pow(10, -9) * math:pow(3000*(lists:nth(0 + 1, X)-32768)/32768, 3) - 7.37*math:pow(10, -6) * math:pow(3000*(lists:nth(0 + 1, X)-32768)/32768, 2) + 6.69*math:pow(10, -3) * (3000*(lists:nth(0 + 1, X)-32768)/32768) - 1.92 end,
          unit => <<"m³⋅m⁻³"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("0202df000393710c60")]),
  io:format("~p~n", [decode("0202df00020c60")]).
