...
class DL_10HS_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(1, new SensorValue[] {
            new SensorValue("Raw sensor reading", "mV", new Conversion() {
                public double execute(double x[]) { return 3 * (x[0] - 32768) / 32768 * 1000; }
            }),
            new SensorValue("Volumetric water content", "m³⋅m⁻³", new Conversion() {
                public double execute(double x[]) { return 2.97*Math.pow(10, -9) * Math.pow(3000*(x[0]-32768)/32768, 3) - 7.37*Math.pow(10, -6) * Math.pow(3000*(x[0]-32768)/32768, 2) + 6.69*Math.pow(10, -3) * (3000*(x[0]-32768)/32768) - 1.92; }
            })
        }),
        new Sensor(1, new SensorValue[] {
...
public class DL_10HS {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_10HS_Definition.SENSORS, pl);
