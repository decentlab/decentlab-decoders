    ...
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(3, new SensorValue[] {
            new SensorValue("Dielectric permittivity", null, new Conversion() {
                public double execute(double x[]) { return Math.pow(0.000000002887 * Math.pow(x[0]/10, 3) - 0.0000208 * Math.pow(x[0]/10, 2) + 0.05276 * (x[0]/10) - 43.39, 2); }
            }),
            new SensorValue("Volumetric water content", "m³⋅m⁻³", new Conversion() {
                public double execute(double x[]) { return x[0]/10 * 0.0003879 - 0.6956; }
            }),
...
public class DL_TRS12 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_TRS12_Definition.SENSORS, pl);
