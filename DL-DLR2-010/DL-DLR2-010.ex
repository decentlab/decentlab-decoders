  ...
  defp sensor_defs do
    [
      %{
        ...
        values: [
          %{
            :name => "CH0: Pulse count",
            :convert => fn x -> Enum.at(x, 0) end,
            :unit => nil
          },
          %{
            :name => "CH0: Pulse interval",
            :convert => fn x -> Enum.at(x, 1) end,
            :unit => "s"
          },
          %{
            :name => "CH0: Cumulative pulse count",
      ...
      %{
        ...
        values: [
          %{
            :name => "CH1: Pulse count",
            :convert => fn x -> Enum.at(x, 0) end,
            :unit => nil
          },
          %{
            :name => "CH1: Pulse interval",
            :convert => fn x -> Enum.at(x, 1) end,
            :unit => "s"
          },
          %{
            :name => "CH1: Cumulative pulse count",
            :convert => fn x -> (Enum.at(x, 2) + Enum.at(x, 3) * 65536) end,
            :unit => nil
          }
        ]
...
IO.inspect(DecentlabDecoder.decode("02198f0007000402580bf0000100000258dece00000c33", :hex))
IO.inspect(DecentlabDecoder.decode("02198f00040c33", :hex))
