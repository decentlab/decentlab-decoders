  ...
  [
    #{
      ...
      values => [
        #{
          name => <<"Voltage ratio">>,
          convert => fun(X) -> ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) / 2 end
        },
        #{
          name => <<"Thermistor resistance">>,
          convert => fun(X) -> 1000 / (((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) / 2) - 41000 end,
          unit => <<"Ω"/utf8>>
        },
        #{
          name => <<"Temperature">>,
          convert => fun(X) -> (1 / (0.0008271111 + 0.000208802 * math:log(1000 / (((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) / 2) - 41000) + 0.000000080592 * math:pow(math:log(1000 / (((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) / 2) - 41000), 3) )) - 273.15 end,
          unit => <<"°C"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("0230c50003a40c00810c60")]),
  io:format("~p~n", [decode("0230c500020c60")]).
