...
local SENSORS = {
  {["length"] = 2,
   ["values"] = {
     {["name"] = "voltage_ratio",
      ["display_name"] = "Voltage ratio",
      ["convert"] = function (x) return ((x[0 + 1] + x[1 + 1]*65536) / 8388608 - 1) / 2 end},
     {["name"] = "thermistor_resistance",
      ["display_name"] = "Thermistor resistance",
      ["convert"] = function (x) return 1000 / (((x[0 + 1] + x[1 + 1]*65536) / 8388608 - 1) / 2) - 41000 end,
      ["unit"] = "Ω"},
     {["name"] = "temperature",
      ["display_name"] = "Temperature",
      ["convert"] = function (x) return (1 / (0.0008271111 + 0.000208802 * math.log(1000 / (((x[0 + 1] + x[1 + 1]*65536) / 8388608 - 1) / 2) - 41000) + 0.000000080592 * math.pow(math.log(1000 / (((x[0 + 1] + x[1 + 1]*65536) / 8388608 - 1) / 2) - 41000), 3) )) - 273.15 end,
      ["unit"] = "°C"}
   }},
  {["length"] = 1,
   ["values"] = {
