...
class DL_DLR2_002_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(4, new SensorValue[] {
            new SensorValue("Pulse count", null, new Conversion() {
                public double execute(double x[]) { return x[0]; }
            }),
            new SensorValue("Pulse interval", "s", new Conversion() {
                public double execute(double x[]) { return x[1]; }
            }),
            new SensorValue("Cumulative pulse count", null, new Conversion() {
...
public class DL_DLR2_002 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_DLR2_002_Definition.SENSORS, pl);
