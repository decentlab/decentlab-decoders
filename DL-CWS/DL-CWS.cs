...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(6, new List<SensorValue>() {
      new SensorValue("Surface temperature", "°C", x => (x[0] - 32768) / 100),
      new SensorValue("Air temperature", "°C", x => (x[1] - 32768) / 100),
      new SensorValue("Air humidity", "%", x => (x[2] - 32768) / 100),
      new SensorValue("Dew point", "°C", x => (x[3] - 32768) / 100),
      new SensorValue("Angle", "°", x => (x[4] - 32768)),
      new SensorValue("Sensor temperature", "°C", x => (x[5] - 32768) / 100)
    }),
