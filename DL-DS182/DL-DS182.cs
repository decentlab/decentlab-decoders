...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(2, new List<SensorValue>() {
      new SensorValue("CH0: Temperature", "°C", x => (x[0] - 32768) / 16),
      new SensorValue("CH0: ID", null, x => x[1])
    }),
    new Sensor(2, new List<SensorValue>() {
      new SensorValue("CH1: Temperature", "°C", x => (x[0] - 32768) / 16),
      new SensorValue("CH1: ID", null, x => x[1])
    }),
