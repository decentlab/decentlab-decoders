...
class DL_ZN1_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Dendrometer position", "µm", new Conversion() {
                public double execute(double x[]) { return ((x[0] + x[1]*65536) / 8388608 - 1) * 20000; }
            })
        }),
...
public class DL_ZN1 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_ZN1_Definition.SENSORS, pl);
