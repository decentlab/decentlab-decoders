  ...
  [
    #{
      ...
      values => [
        #{
          name => <<"Pressure">>,
          convert => fun(X) -> ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) / 64 / 0.005 * ?Pmax end,
          unit => <<"bar"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("02319e000376e400800c94")]),
  io:format("~p~n", [decode("02319e00020c94")]).
