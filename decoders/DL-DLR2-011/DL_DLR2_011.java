...
class DL_DLR2_011_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(1, new SensorValue[] {
            new SensorValue("CH0: Input", null, new Conversion() {
                public double execute(double x[]) { return x[0]; }
            })
        }),
        new Sensor(1, new SensorValue[] {
            new SensorValue("CH1: Input", null, new Conversion() {
                public double execute(double x[]) { return x[0]; }
            })
        }),
        new Sensor(1, new SensorValue[] {
...
public class DL_DLR2_011 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_DLR2_011_Definition.SENSORS, pl);
