...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(2, new List<SensorValue>() {
      new SensorValue("Voltage ratio", null, x => ((x[0] + x[1]*65536) / 8388608 - 1) / 2),
      new SensorValue("Thermistor resistance", "Ω", x => 1000 / (((x[0] + x[1]*65536) / 8388608 - 1) / 2) - 41000),
      new SensorValue("Temperature", "°C", x => (1 / (0.0008271111 + 0.000208802 * Math.Log(1000 / (((x[0] + x[1]*65536) / 8388608 - 1) / 2) - 41000) + 0.000000080592 * Math.Pow(Math.Log(1000 / (((x[0] + x[1]*65536) / 8388608 - 1) / 2) - 41000), 3) )) - 273.15)
    }),
