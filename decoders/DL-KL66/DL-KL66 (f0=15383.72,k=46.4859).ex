    ...
    [
      %{
        ...
        values: [
          ...
          %{
            :name => "Weight",
            :convert => fn x -> (:math.pow(Enum.at(x, 0) / Enum.at(x, 1) * 32768, 2) - :math.pow(@f0, 2)) * @k / 1000000 end,
            :unit => "g"
          },
          %{
            :name => "Elongation",
            :convert => fn x -> (:math.pow(Enum.at(x, 0) / Enum.at(x, 1) * 32768, 2) - :math.pow(@f0, 2)) * @k / 1000000 * (-1.5) / 1000 * 9.8067 end,
            :unit => "µm"
          },
          %{
            :name => "Strain",
            :convert => fn x -> (:math.pow(Enum.at(x, 0) / Enum.at(x, 1) * 32768, 2) - :math.pow(@f0, 2)) * @k / 1000000 * (-1.5) / 1000 * 9.8067 / 0.066 end,
            :unit => "µm⋅m⁻¹"
          }
        ]
      },
