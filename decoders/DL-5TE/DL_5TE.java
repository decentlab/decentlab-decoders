    ...
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(3, new SensorValue[] {
            new SensorValue("Dielectric permittivity", null, new Conversion() {
                public double execute(double x[]) { return x[0] / 50; }
            }),
            new SensorValue("Volumetric water content", "m³⋅m⁻³", new Conversion() {
                public double execute(double x[]) { return 0.0000043 * Math.pow(x[0]/50, 3) - 0.00055 * Math.pow(x[0]/50, 2) + 0.0292 * (x[0]/50) - 0.053; }
            }),
            new SensorValue("Soil temperature", "°C", new Conversion() {
                public double execute(double x[]) { return (x[1] - 400) / 10; }
            }),
...
public class DL_5TE {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_5TE_Definition.SENSORS, pl);
