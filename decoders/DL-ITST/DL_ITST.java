...
class DL_ITST_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Temperature target", "°C", new Conversion() {
                public double execute(double x[]) { return (x[0] - 1000) / 10; }
            }),
            new SensorValue("Temperature head", "°C", new Conversion() {
...
public class DL_ITST {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_ITST_Definition.SENSORS, pl);
