    ...
    #{
      ...
      values => [
        #{
          ...
          convert => fun(X) -> ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) * 2000 / (1 - ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1)) end,
          unit => <<"Ω"/utf8>>
        },
        #{
          name => <<"Temperature">>,
          convert => fun(X) -> -245.18 + 0.23469 * (((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) * 2000 / (1 - ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1))) + 0.0000104876 * math:pow(((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) * 2000 / (1 - ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1)), 2) end,
          unit => <<"°C"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("024c620003573400ad0ae1")]),
  io:format("~p~n", [decode("024c6200020ae1")]).
