...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(5, new List<SensorValue>() {
      ...
      new SensorValue("Oxygen saturation", "%", x => (x[2] - 32768) / 100),
      new SensorValue("Oxygen concentration", "mg⋅L⁻¹", x => (x[3] - 32768) / 100),
      new SensorValue("Oxygen concentration (alt)", "ppm", x => (x[4] - 32768) / 100)
    }),
