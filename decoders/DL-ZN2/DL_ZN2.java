...
class DL_ZN2_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(4, new SensorValue[] {
            new SensorValue("Dendrometer A Position", "µm", new Conversion() {
                public double execute(double x[]) { return ((x[0] + x[1]*65536) / 8388608 - 1) * 20000; }
            }),
            new SensorValue("Dendrometer B Position", "µm", new Conversion() {
                public double execute(double x[]) { return ((x[0] + x[1]*65536) / 8388608 - (x[2] + x[3]*65536) / 8388608) * 20000; }
            })
        }),
...
public class DL_ZN2 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_ZN2_Definition.SENSORS, pl);
