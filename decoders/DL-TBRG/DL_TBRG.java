    ...
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(4, new SensorValue[] {
            new SensorValue("Precipitation", "mm", new Conversion() {
                public double execute(double x[]) { return x[0] * resolution; }
            }),
            new SensorValue("Precipitation interval", "s", new Conversion() {
                public double execute(double x[]) { return x[1]; }
            }),
            new SensorValue("Cumulative precipitation", "mm", new Conversion() {
                public double execute(double x[]) { return (x[2] + x[3] * 65536) * resolution; }
            })
        }),
...
public class DL_TBRG {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_TBRG_Definition.SENSORS, pl);
