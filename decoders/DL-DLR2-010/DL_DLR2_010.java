...
class DL_DLR2_010_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(4, new SensorValue[] {
            new SensorValue("CH0: Pulse count", null, new Conversion() {
                public double execute(double x[]) { return x[0]; }
            }),
            new SensorValue("CH0: Pulse interval", "s", new Conversion() {
                public double execute(double x[]) { return x[1]; }
            }),
            new SensorValue("CH0: Cumulative pulse count", null, new Conversion() {
                public double execute(double x[]) { return (x[2] + x[3] * 65536); }
            })
        }),
        new Sensor(4, new SensorValue[] {
            new SensorValue("CH1: Pulse count", null, new Conversion() {
                public double execute(double x[]) { return x[0]; }
            }),
            new SensorValue("CH1: Pulse interval", "s", new Conversion() {
                public double execute(double x[]) { return x[1]; }
            }),
            new SensorValue("CH1: Cumulative pulse count", null, new Conversion() {
                public double execute(double x[]) { return (x[2] + x[3] * 65536); }
            })
        }),
...
public class DL_DLR2_010 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_DLR2_010_Definition.SENSORS, pl);
