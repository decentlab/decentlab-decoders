...
class DL_PR21_Definition {

    /* device-specific parameters */
    public static final double Pmin = -1.0;
    public static final double Pmax = 10.0;
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Pressure", "bar", new Conversion() {
                public double execute(double x[]) { return (x[0] - 16384) / 32768 * (Pmax - Pmin) + Pmin; }
            }),
            new SensorValue("Temperature", "°C", new Conversion() {
                public double execute(double x[]) { return (x[1] - 384) * 0.003125 - 50; }
            })
        }),
...
public class DL_PR21 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_PR21_Definition.SENSORS, pl);
