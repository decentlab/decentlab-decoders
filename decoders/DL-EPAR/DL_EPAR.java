    ...
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(1, new SensorValue[] {
            new SensorValue("Photosynthetically active radiation", "µmol⋅m⁻²⋅s⁻¹", new Conversion() {
                public double execute(double x[]) { return 3 * (x[0] / 32768 - 1) / 64 * 1000 * 100; }
            })
        }),
        new Sensor(1, new SensorValue[] {
...
public class DL_EPAR {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_EPAR_Definition.SENSORS, pl);
