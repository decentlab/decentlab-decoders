  ...
  [
    #{
      ...
      values => [
        #{
          name => <<"Pressure">>,
          convert => fun(X) -> (lists:nth(0 + 1, X) - 32768) / ?kp end,
          unit => <<"bar"/utf8>>
        },
        #{
          name => <<"Temperature (electronics)">>,
          convert => fun(X) -> (lists:nth(1 + 1, X) - 32768) / 256 end,
          unit => <<"°C"/utf8>>
        },
        #{
          name => <<"Temperature (PT1000)">>,
          convert => fun(X) -> (lists:nth(2 + 1, X) - 32768) / 256 end,
          unit => <<"°C"/utf8>>
        },
        #{
...
start() ->
  io:format("~p~n", [decode("020a17000380079786978180060c2b")]),
  io:format("~p~n", [decode("020a1700020c2b")]).
