    ...
    #{
      ...
      values => [
        #{
          ...
          convert => fun(X) -> ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - 1) * 20000 end,
          unit => <<"µm"/utf8>>
        },
        #{
          name => <<"Dendrometer B Position">>,
          convert => fun(X) -> ((lists:nth(0 + 1, X) + lists:nth(1 + 1, X)*65536) / 8388608 - (lists:nth(2 + 1, X) + lists:nth(3 + 1, X)*65536) / 8388608) * 20000 end,
          unit => <<"µm"/utf8>>
        }
      ]
    },
...
start() ->
  io:format("~p~n", [decode("0211110003409a00863039003e0c54")]).
