...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(7, new List<SensorValue>() {
      new SensorValue("Photosynthetically active radiation", "µmol⋅m⁻²⋅s⁻¹", x => (x[0] - 32768) / 10),
      new SensorValue("Air temperature", "°C", x => (x[1] - 32768) / 100),
      new SensorValue("Air humidity", "%", x => (x[2] - 32768) / 10),
      new SensorValue("CO2 concentration", "ppm", x => (x[3] - 32768) / 1),
      new SensorValue("Atmospheric pressure", "kPa", x => (x[4] - 32768) / 100),
      new SensorValue("Vapor pressure deficit", "kPa", x => (x[5] - 32768) / 100),
      new SensorValue("Dew point", "°C", x => (x[6] - 32768) / 100)
    }),
