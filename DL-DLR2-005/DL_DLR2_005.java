...
class DL_DLR2_005_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Voltage", "V", new Conversion() {
                public double execute(double x[]) { return 3 * ((x[0] + x[1]*65536) / 8388608 - 1); }
            })
        }),
...
public class DL_DLR2_005 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_DLR2_005_Definition.SENSORS, pl);
