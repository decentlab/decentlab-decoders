...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(3, new List<SensorValue>() {
      ...
      new SensorValue("Weight", "g", x => (Math.Pow(x[0] / x[1] * 32768, 2) - Math.Pow((15383.72), 2)) * (46.4859) / 1000000),
      new SensorValue("Elongation", "µm", x => (Math.Pow(x[0] / x[1] * 32768, 2) - Math.Pow((15383.72), 2)) * (46.4859) / 1000000 * (-1.5) / 1000 * 9.8067),
      new SensorValue("Strain", "µm⋅m⁻¹", x => (Math.Pow(x[0] / x[1] * 32768, 2) - Math.Pow((15383.72), 2)) * (46.4859) / 1000000 * (-1.5) / 1000 * 9.8067 / 0.066)
    }),
