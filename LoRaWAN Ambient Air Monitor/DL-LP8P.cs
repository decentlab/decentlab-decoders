...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    ...
    new Sensor(2, new List<SensorValue>() {
      new SensorValue("Barometer temperature", "°C", x => (x[0] - 5000) / 100),
      new SensorValue("Barometric pressure", "Pa", x => x[1] * 2)
    }),
    new Sensor(8, new List<SensorValue>() {
      new SensorValue("CO2 concentration", "ppm", x => x[0] - 32768),
      new SensorValue("CO2 concentration LPF", "ppm", x => x[1] - 32768),
      new SensorValue("CO2 sensor temperature", "°C", x => (x[2] - 32768) / 100),
      new SensorValue("Capacitor voltage 1", "V", x => x[3] / 1000),
      new SensorValue("Capacitor voltage 2", "V", x => x[4] / 1000),
      new SensorValue("CO2 sensor status", null, x => x[5]),
      new SensorValue("Raw IR reading", null, x => x[6]),
      new SensorValue("Raw IR reading LPF", null, x => x[7])
    }),
