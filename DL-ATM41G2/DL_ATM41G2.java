    ...
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(17, new SensorValue[] {
            ...
            new SensorValue("Precipitation", "mm", new Conversion() {
                public double execute(double x[]) { return x[1] / 1000; }
            }),
            new SensorValue("Lightning strike count", "None", new Conversion() {
            ...
            new SensorValue("Barometric pressure", "kPa", new Conversion() {
            ...
            new SensorValue("Internal temperature", "°C", new Conversion() {
                public double execute(double x[]) { return (x[11] - 32768) / 10; }
            }),
            new SensorValue("Tilt angle, X orientation", "°", new Conversion() {
                public double execute(double x[]) { return (x[12] - 32768) / 10; }
            }),
            new SensorValue("Tilt angle, Y orientation", "°", new Conversion() {
                public double execute(double x[]) { return (x[13] - 32768) / 10; }
            }),
            new SensorValue("Precipitation electrical conductivity", "µS⋅cm⁻¹", new Conversion() {
                public double execute(double x[]) { return x[14] - 32768; }
            }),
            new SensorValue("Cumulative precipitation", "mm", new Conversion() {
                public double execute(double x[]) { return (x[15] + x[16] * 65536) / 1000; }
            })
        }),
...
public class DL_ATM41G2 {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_ATM41G2_Definition.SENSORS, pl);
