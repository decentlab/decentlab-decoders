...
class DL_OPTOD_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(5, new SensorValue[] {
            ...
            new SensorValue("Oxygen saturation", "%", new Conversion() {
                public double execute(double x[]) { return (x[2] - 32768) / 100; }
            }),
            new SensorValue("Oxygen concentration", "mg⋅L⁻¹", new Conversion() {
                public double execute(double x[]) { return (x[3] - 32768) / 100; }
            }),
            new SensorValue("Oxygen concentration (alt)", "ppm", new Conversion() {
...
public class DL_OPTOD {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_OPTOD_Definition.SENSORS, pl);
