...
class DL_PYR_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(1, new SensorValue[] {
            new SensorValue("Total solar radiation", "W⋅m⁻²", new Conversion() {
                public double execute(double x[]) { return 3 * (x[0] / 32768 - 1) * 1000 * 5; }
            })
        }),
        new Sensor(1, new SensorValue[] {
...
public class DL_PYR {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_PYR_Definition.SENSORS, pl);
