...
class DL_NTU_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(5, new SensorValue[] {
            ...
            new SensorValue("Turbidity in NTU", "NTU", new Conversion() {
                public double execute(double x[]) { return x[2] / 10; }
            }),
            new SensorValue("Turbidity in FNU", "FNU", new Conversion() {
                public double execute(double x[]) { return x[3] / 10; }
            }),
            new SensorValue("Turbidity in mg/L", "mg⋅L⁻¹", new Conversion() {
...
public class DL_NTU {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_NTU_Definition.SENSORS, pl);
