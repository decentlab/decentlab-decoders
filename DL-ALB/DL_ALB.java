...
class DL_ALB_Definition {
    public static final Sensor SENSORS[] = new Sensor[] {
        new Sensor(2, new SensorValue[] {
            new SensorValue("Incoming radiation", "W⋅m⁻²", new Conversion() {
                public double execute(double x[]) { return (x[0] - 32768) / 10; }
            }),
            new SensorValue("Reflected radiation", "W⋅m⁻²", new Conversion() {
                public double execute(double x[]) { return (x[1] - 32768) / 10; }
            }),
            new SensorValue("albedo", null, new Conversion() {
                public double execute(double x[]) { return ((((x[0] - 32768) > 0) && ((x[1] - 32768) > 0)) ?  (x[1] - 32768) / (x[0] - 32768) :  0); }
            })
        }),
...
public class DL_ALB {
    public static void main(String[] args) {
        ...
        for (String pl : payloads) {
            try {
                Map<String, DecodedValue> decoded = DecentlabDecoder.decode(DL_ALB_Definition.SENSORS, pl);
