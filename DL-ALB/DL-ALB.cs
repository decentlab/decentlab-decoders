...
public class DecentlabDecoder
{
  ...
  private static readonly List<Sensor> SENSORS = new List<Sensor>() {
    new Sensor(2, new List<SensorValue>() {
      new SensorValue("Incoming radiation", "W⋅m⁻²", x => (x[0] - 32768) / 10),
      new SensorValue("Reflected radiation", "W⋅m⁻²", x => (x[1] - 32768) / 10),
      new SensorValue("albedo", null, x => ((((x[0] - 32768) > 0) && ((x[1] - 32768) > 0)) ?  (x[1] - 32768) / (x[0] - 32768) :  0))
    }),
